...
if (slides.length > 0) {
    // Start slideshow after page load
    setTimeout(() => {
...
const pageSlides = document.querySelectorAll('.page-header-slide');
const totalPageSlides = pageSlides.length;
...
function nextPageSlide() {
    currentPageSlide = (currentPageSlide + 1) % totalPageSlides;
    showPageSlide(currentPageSlide);
}

// Initialize page header slideshow
if (pageSlides.length > 0) {
    // Start slideshow after page load
    setTimeout(() => {
        setInterval(nextPageSlide, 5000); // Change slide every 5 seconds
    }, 1000);
}
